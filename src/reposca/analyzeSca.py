...
import re
import jsonpath
import pyrpm.spec as Spec
import os
import sys
sys.path.append("..")
import reposca.licenseCheck as licCheck
import util.catchUtil as catch
import util.postOrdered as inpost
...
class Analyze(object):
    ...
    @catch.catch_error
    def getScaAnalyze(self, scaJson, anlyzeSrc, type, copyright_type, file_array):
        '''
        ...
        fileLicenseCheck = licCheck.LicenseCheck('file', 'indelic')
        licenseCheck = licCheck.LicenseCheck('repo', 'indelic')
        pathDepth = 3
        if type == 'ref':
            pathDepth = 4
        ...
        for i, var in enumerate(licenseList):   
            ...
            if self.checkNotice(path, pathDepth) and len(copyrightList[i]) > 0:
            ...
            if self.is_in(path, file_array) and copyright_type == 'Huawei':
                if len(copyrightList[i]) > 0:
                    copyrightInfo = copyrightList[i]
                    for info in copyrightInfo:
                        if self.copyright_check(info['copyright']) is None:
                            isCopyright = False
            ...
            if path.endswith((".spec",)) and self.checkPath(path, 2):
                ...
                try:
                    ...
                    if  specLic is not None:
                        if 'all_license' in specLic:
                            licenses = inpost.infixToPostfix(spec.all_license)
                            specLic = spec.all_license
                        else:
                            licenses = inpost.infixToPostfix(specLic)
            ...
            for pathLicense in var:
                ...
                if 'LicenseRef-scancode-' in spdx_name:
                    if "public-domain" in spdx_name:
                        ...
                        continue
                spdxLicenses = inpost.infixToPostfix(spdx_name)
                # Determine whether there is a repo license
                if self.checkRepoLicense(path, pathDepth) and specFlag:
    ...
    @catch.catch_error
    def checkNotice(slef, path, depth):
        # Check the notice file
        path = path.lower()
    ...
    @catch.catch_error
    def checkRepoLicense(self, path, depth):
        path = path.lower()
    ...
    @catch.catch_error
    def is_in(self, path, file_array):
    ...
    @catch.catch_error
    def copyright_check(self, copyright):
